  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (requires2FA) {
        try {
          ...
          if (data.user && typeof data.user.username === 'string') {
            // localStorage holds non-sensitive UI hints only (id/username/name
            // para greet no topbar antes do /me/ resolver). `role` NUNCA é
            // persistido: a UI lê role via useCurrentUser() (GET /me/) e o
            // backend reavalia permissões em todo request.
      ...
      try {
        ...
        // de UI sem peso de autorização (id/username/first_name/last_name).
        // `role` NUNCA é persistido — a UI lê via useCurrentUser() (GET /me/)
        // e o backend reavalia permissões em todo request.
        if (data.user && typeof data.user.username === 'string') {
          const { id, username, first_name, last_name } = data.user;
          localStorage.setItem('user', JSON.stringify({ id, username, first_name, last_name }));
        }
