...
export default function ConfigFinanceiro({ isDemoMode }: { isDemoMode: boolean }) {
  ...
  useEffect(() => { fetchConfig().then(() => fetchTaxConfig()); }, [fetchConfig]);
  ...
  const handleAddPartner = async () => {
    if (!config) { toast.error('Salve a configuração de distribuição primeiro.'); return; }
    if (!newPartner.name || !newPartner.share_pct) {
      toast.error('Preencha nome e percentual do sócio.');
      return;
    }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h2 className="text-lg font-semibold text-gray-900 dark:text-gray-100">Distribuição de Lucros</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400">Configure os percentuais de distribuição e sócios</p>
        </div>
      </div>
      ...
      <ConfirmDialog
        ...
        title="Remover Sócio"
        description={`Deseja remover o sócio "${confirmDeletePartner?.name}"?`}
