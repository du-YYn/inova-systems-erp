...
export default function DashboardComercial() {
  ...
  const [stats, setStats] = useState({ mrr: 0, active_contracts: 0, expiring_contracts: 0, proposals_sent: 0, proposals_value: 0, proposals_approved: 0 });
  ...
  const cards = [
    { label: 'MRR', value: fmt(stats.mrr), icon: DollarSign, color: 'text-green-600', bg: 'bg-green-50 dark:bg-green-900/30' },
    { label: 'Leads no Funil', value: totalLeads, icon: Users, color: 'text-blue-600', bg: 'bg-blue-50 dark:bg-blue-900/30' },
    { label: 'Taxa Conversão', value: `${winRate}%`, icon: Target, color: 'text-accent-gold', bg: 'bg-amber-50 dark:bg-amber-900/30' },
    { label: 'Ticket Médio', value: fmt(avgTicket), icon: TrendingUp, color: 'text-purple-600', bg: 'bg-purple-50 dark:bg-purple-900/30' },
    { label: 'Contratos Ativos', value: stats.active_contracts, icon: Briefcase, color: 'text-blue-600', bg: 'bg-blue-50 dark:bg-blue-900/30' },
    { label: 'Propostas Enviadas', value: stats.proposals_sent, icon: FileText, color: 'text-amber-600', bg: 'bg-amber-50 dark:bg-amber-900/30' },
    { label: 'Propostas Aprovadas', value: stats.proposals_approved, icon: ArrowUpRight, color: 'text-green-600', bg: 'bg-green-50 dark:bg-green-900/30' },
          ...
          {pipeline.length === 0 ? <p className="text-sm text-gray-400 py-8 text-center">Sem dados</p> : (
            <ResponsiveContainer width="100%" height={250}>
              <BarChart data={pipeline}>
                ...
                <Bar dataKey="count" name="Leads" fill="#A6864A" radius={[4, 4, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          )}
        </div>
