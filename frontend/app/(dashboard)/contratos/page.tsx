...
const statusColors: Record<string, string> = {
  draft: 'bg-gray-100 dark:bg-gray-700 text-gray-700',
  pending_signature: 'bg-yellow-100 dark:bg-yellow-900/40 text-yellow-800',
  active: 'bg-green-100 dark:bg-green-900/40 text-green-800',
  suspended: 'bg-orange-100 dark:bg-orange-900/40 text-orange-800',
  cancelled: 'bg-red-100 dark:bg-red-900/40 text-red-800',
...
export default function ContratosPage() {
  ...
  const isDirty = useMemo(() => {
    return formData.title !== '' || formData.customer !== '' ||
      formData.start_date !== '' || formData.end_date !== '' ||
      formData.monthly_value !== '' || formData.hourly_rate !== '' ||
      formData.total_hours_monthly !== '' || formData.notes !== '' ||
      formData.terms !== '' || formData.auto_renew !== false;
  }, [formData]);
  ...
  return (
    <div className="p-8">
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-2xl font-semibold text-gray-900 dark:text-gray-100">Contratos</h1>
          <p className="text-gray-500 dark:text-gray-400 mt-1">Gestão de contratos com clientes</p>
        </div>
        <button
      ...
      <div className="card overflow-hidden">
        {loading ? (
          ...
          <div className="text-center py-16 text-gray-500 dark:text-gray-400">
            <ScrollText className="w-12 h-12 mx-auto mb-3 text-gray-300 dark:text-gray-500" />
          ...
          <table className="w-full">
            <thead className="bg-gray-50 dark:bg-gray-700/50 border-b border-gray-100 dark:border-gray-700">
              <tr>
                ...
                <th className="hidden md:table-cell text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider px-6 py-3">Cliente</th>
                <th className="hidden md:table-cell text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider px-6 py-3">Tipo / Cobrança</th>
                <th className="hidden md:table-cell text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider px-6 py-3">Valor</th>
                <th className="hidden md:table-cell text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider px-6 py-3">Vigência</th>
            ...
            <tbody className="divide-y divide-gray-50 dark:divide-gray-700">
              {contracts.map((c) => (
                <tr key={c.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                  ...
                  <td className="px-6 py-4">
                    <span className={`px-2 py-1 rounded-full text-xs font-medium ${statusColors[c.status] || 'bg-gray-100 dark:bg-gray-700 text-gray-700'}`}>
                    ...
                    <div className="flex items-center gap-1 justify-end">
                      {c.status === 'pending_signature' && (
                        <button
                          ...
                          className="px-2 py-1 text-xs bg-green-50 dark:bg-green-900/30 text-green-700 rounded hover:bg-green-100 transition-colors"
                        >
                          Ativar
                        </button>
                      )}
                      {c.status === 'active' && (
                        <button
                          ...
                          className="px-2 py-1 text-xs bg-red-50 dark:bg-red-900/30 text-red-700 rounded hover:bg-red-100 transition-colors"
                        >
                          Cancelar
                        </button>
                      )}
          ...
          <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 w-full max-w-lg mx-4 max-h-[90vh] overflow-y-auto shadow-modal animate-modal-in">
            ...
            <form onSubmit={handleCreate} className="space-y-4">
              <FormField label="Título" required error={errors.title}>
                {(props) => (
                  <input
                    type="text" {...props} value={formData.title}
              ...
              <div className="grid grid-cols-2 gap-4">
                <FormField label="Tipo">
                  {(props) => (
                    <select
                      {...props} value={formData.contract_type}
                ...
                <FormField label="Cobrança">
                  {(props) => (
                    <select
                      {...props} value={formData.billing_type}
