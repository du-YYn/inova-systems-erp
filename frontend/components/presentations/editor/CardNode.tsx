...
import type { CardNode as CardNodeT } from '@/lib/presentations/types';
...
function CardNodeComponent({ data, selected }: NodeProps<CardNodeT>) {
  const meta = CARD_TYPE_MAP[data.tipo];
  ...
  return (
    <div
      className={clsx(
        'rounded-lg bg-[color:var(--pr-bg-elevated)] min-w-[220px] max-w-[280px] transition-shadow',
        selected ? 'shadow-[0_0_0_2px_var(--pr-gold)]' : 'shadow-[0_2px_10px_rgba(0,0,0,0.5)]',
      )}
      style={{ border: `${BORDER[data.borda]} solid ${cor}` }}
    >
      ...
      <div className="px-3 py-3">
        <div className="text-sm font-medium text-[color:var(--pr-text-primary)] leading-tight">
          {data.titulo || <span className="text-[color:var(--pr-text-tertiary)] italic">Sem título</span>}
        </div>
        {data.descricao && (
      ...
      {(['Top', 'Right', 'Bottom', 'Left'] as const).map((side) => (
        <Handle
          ...
          className="!w-2 !h-2 !bg-[color:var(--pr-gold)] !border-[color:var(--pr-bg)]"
        />
      ))}
      {(['Top', 'Right', 'Bottom', 'Left'] as const).map((side) => (
        <Handle
