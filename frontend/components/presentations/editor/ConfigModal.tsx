...
import type { ConfigJson } from '@/lib/presentations/types';
...
export function ConfigModal({ open, onClose, config, onChange }: Props) {
  return (
    <Modal
      ...
      onClose={onClose}
      footer={<PButton variant="ghost" onClick={onClose}>Fechar</PButton>}
    >
      <div className="flex flex-col gap-5 min-w-[380px]">
        <div className="flex flex-col gap-1.5">
          <label className="pr-label-caps">Cor de fundo</label>
          <input
            type="color" value={config.cor_fundo}
            onChange={(e) => onChange({ cor_fundo: e.target.value })}
            className="w-full h-10 rounded border border-[color:var(--pr-border)] bg-transparent cursor-pointer"
          />
        </div>

        <div className="flex flex-col gap-1.5">
          <label className="pr-label-caps">Cor de acento</label>
          <input
            type="color" value={config.cor_acento}
            onChange={(e) => onChange({ cor_acento: e.target.value })}
            className="w-full h-10 rounded border border-[color:var(--pr-border)] bg-transparent cursor-pointer"
          />
          <span className="text-[11px] text-[color:var(--pr-text-tertiary)]">
            Aplicada em destaques e CTAs durante apresentação
          </span>
        </div>

        <div className="flex flex-col gap-1.5">
          <label className="pr-label-caps">URL do logo do cliente</label>
          <input
            type="text"
        ...
        <div className="flex flex-col gap-1.5">
          <label className="pr-label-caps">Posição do logo</label>
