...
import { CARD_TIPO_MAP } from "@/editor/cardTypes";
...
function variants(efeito: PlayerNodeData["efeito"]): Variants {
...
function PlayerCardNodeComponent({ data }: NodeProps<PlayerNode>) {
  ...
  const meta = CARD_TIPO_MAP[original.tipo];
  ...
  return (
    <motion.div
      ...
      className="rounded-lg bg-[color:var(--color-bg-elevated)] min-w-[220px] max-w-[280px] shadow-[0_6px_30px_rgba(0,0,0,0.6)]"
      style={{ border: `${BORDA[original.borda]} solid ${cor}` }}
    >
