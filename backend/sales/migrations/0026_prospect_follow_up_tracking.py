...
class Migration(migrations.Migration):
    ...
    quebrando o CI (column does not exist) e possivelmente exigiu adição
    manual das colunas em produção.

    Usa RunSQL com `IF NOT EXISTS` para ser seguro em qualquer estado do
    banco:
    - Se as colunas já existem → ALTER é no-op, migration apenas registra
      o estado no Django.
    - Se as colunas não existem → ALTER cria, populando com DEFAULT.
